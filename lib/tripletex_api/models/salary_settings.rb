...
module TripletexApi
  ...
  class SalarySettings
    ...
    def list_invalid_properties
      ...
      if @payroll_tax_calc_method.nil?
        invalid_properties.push("invalid value for 'payroll_tax_calc_method', payroll_tax_calc_method cannot be nil.")
      end

      if @payroll_tax_calc_method.to_s.length > 2
    ...
    def valid?
      return false if @payroll_tax_calc_method.nil?
      payroll_tax_calc_method_validator = EnumAttributeValidator.new('String', ["AA", "BB", "CC", "DD", "EE", "GG", "JJ", "EMPTY"])
      return false unless payroll_tax_calc_method_validator.valid?(@payroll_tax_calc_method)
      return false if @payroll_tax_calc_method.to_s.length > 2
      return true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] payroll_tax_calc_method Object to be assigned
    def payroll_tax_calc_method=(payroll_tax_calc_method)
      validator = EnumAttributeValidator.new('String', ["AA", "BB", "CC", "DD", "EE", "GG", "JJ", "EMPTY"])
      unless validator.valid?(payroll_tax_calc_method)
        fail ArgumentError, "invalid value for 'payroll_tax_calc_method', must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [id, version, changes, url, municipality, payroll_tax_calc_method].hash
    end
