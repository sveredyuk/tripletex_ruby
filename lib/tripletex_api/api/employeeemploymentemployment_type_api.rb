...
module TripletexApi
  class EmployeeemploymentemploymentTypeApi
    ...
    # @option opts [String] :type maritimeEmploymentType
    # @option opts [Integer] :from From index (default to 0)
    # @option opts [Integer] :count Number of elements to return (default to 1000)
    # @option opts [String] :sorting Sorting pattern
    # @option opts [String] :fields Fields filter pattern
    # @return [ListResponseEmploymentType]
    def get_maritime_employment_type(opts = {})
      data, _status_code, _headers = get_maritime_employment_type_with_http_info(opts)
      return data
    end
    ...
    def get_maritime_employment_type_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: EmployeeemploymentemploymentTypeApi.get_maritime_employment_type ..."
      end
      if @api_client.config.client_side_validation && opts[:'type'] && !['SHIP_REGISTER', 'SHIP_TYPE', 'TRADE_AREA'].include?(opts[:'type'])
